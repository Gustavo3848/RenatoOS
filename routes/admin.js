    ...
    if (filtro > 0) {
                ...
                OS.find({ dataData: { $gte: req.body.datain, $lte: req.body.datafinal }, pagamento: "FINALIZADO" }).populate("cliente").lean().then(function (os) {
                    Cliente.find().lean().then(function (clientes) {
                        ...
                        while (cont < os.length) {
                            ...
                            os[cont].valorString = aux.valor(os[cont].valorString)
                            cont++
                        }
                        var textoValor = '' + valor.toFixed(2)
                        textoValor = aux.valor(textoValor)
                        res.render("pdfs/relatorio_pagamento", { layout: "other", orcamentos: os, total: textoValor}, function (erro, html) {
                        ...
                        res.render("admin/relatorio/pagamentos", { cliente: clientes, orcamentos: os, soma: textoValor, teste: os })
                    }).catch((function (err) {
                        console.log(err)
                        res.redirect("/admin/relatoriopagamentos")
                    }))
        ...
        OS.find({ cliente: req.body.idCliente, dataData: { $gte: req.body.datain, $lte: req.body.datafinal }}).populate("cliente").lean().then(function (os) {
            Cliente.find().lean().then(function (clientes) {
                ...
                var textoValor = '' + valor.toFixed(2)
                textoValor = aux.valor(textoValor)
                res.render("pdfs/relatoriopagamento", { layout: "other", orcamentos: os, total: textoValor, cliente: cliente }, function (erro, html) {
                    pdf.create(html, {}).toFile("./pdfs/relatorios/relatorio.pdf", (function (erro, res) {
                        if (erro) {
                            console.log(erro)
                        } else {
                ...
                res.render("pdfs/relatoriodetalhado", { layout: "other", orcamentos: os, total:textoValor, cliente: cliente, dataincial: datainicial, datafinal: dataifinal }, function (erro, html) {
                    if (erro) {
                        console.log(erro)
...
router.get("/orcamentos", function (req, res) {
    OS.find({status:"PENDENTE"}).sort({numeroOS:'desc'}).populate("cliente").lean().then(function (orcamentos) {
        Cliente.find().sort({nome:'asc'}).lean().then(function (cliente) {
        ...
        Servico.find({ os: orcamento._id }).populate("os").lean().then(function (servicos) {
            Material.find({ os: orcamento._id }).populate("os").lean().then(function (materiais) {
                ...
                while (cont < servicos.length) {
                    valorString = servicos[cont].valor
                    valorHoraString = servicos[cont].valorHora
                    servicos[cont].valorString = br_format.NumberFormat('pt-br',{ style: 'currency', currency: 'BRL' }).format(valorString)
                    servicos[cont].valorHoraString = br_format.NumberFormat('pt-br',{ style: 'currency', currency: 'BRL' }).format(valorHoraString)
                    cont++
                }
                cont = 0;
                while (cont < materiais.length) {
                    valorMaterialString = materiais[cont].valor
                    materiais[cont].valorString = br_format.NumberFormat('pt-br',{ style: 'currency', currency: 'BRL' }).format(valorMaterialString)
                    cont++
                }
        ...
        Servico.find({ os: orcamento._id }).populate("os").lean().then(function (servicos) {
            Material.find({ os: orcamento._id }).populate("os").lean().then(function (materiais) {
                ...
                while (cont < servicos.length) {
                    valorString = servicos[cont].valor
                    valorHoraString = servicos[cont].valorHora
                    //servicos[cont].valorString = valorString.toLocaleString("pt-BR",{ style: 'currency', currency: 'BRL' })
                    servicos[cont].valorString = br_format.NumberFormat('pt-br',{ style: 'currency', currency: 'BRL' }).format(valorString)
                    servicos[cont].valorHoraString = br_format.NumberFormat('pt-br',{ style: 'currency', currency: 'BRL' }).format(valorHoraString)
                    cont++
                }
                ...
                res.render("pdfs/ordemFinal", { layout: "other", orcamento: orcamento, servicos: servicos, materiais: materiais, total: valor }, function (erro, html) {
    ...
    OS.findById(req.params.id).populate("cliente").lean().then(function (orcamento) {
        Servico.find({ os: orcamento._id }).populate("os").lean().then(function (servicos) {
            Material.find({ os: orcamento._id }).populate("os").lean().then(function (materiais) {
                ...
                var valor = parseFloat(req.params.valor)
                valor = br_format.NumberFormat('pt-br',{ style: 'currency', currency: 'BRL' }).format(valor)
                res.render("pdfs/ordemFinalSemHora", { layout: "other", orcamento: orcamento, servicos: servicos, materiais: materiais, total:valor  }, function (erro, html) {
                    pdf.create(html, {}).toFile("./pdfs/osfinal/" + numeroOS + ".pdf", (function (erro, res) {
